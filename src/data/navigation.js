...
/**
 * Eric's console is a THIRD mode, beside the live console and Setup.
 *
 * It is a frozen copy of an earlier product — a UK bank-to-bank participant
 * console — kept whole so it can still be demonstrated. It shares this app's
 * shell and components but none of its data or vocabulary, which is why it
 * gets its own rail rather than a section in this one.
 */
export const isEricPath = (pathname) => pathname === '/eric' || pathname.startsWith('/eric/');

export const navTreeFor = (pathname, ericTree) => {
  if (isEricPath(pathname)) return ericTree ?? [];
