...
function FlaggedTab() {
  ...
  const columns = [
    ...
    { key: 'approvedAmount', header: 'Approved', fw: 8, align: 'right', sortable: true, cell: (r) => (r.approvedAmount ? <Money value={r.approvedAmount} /> : <NotApplicable />), text: (r) => moneyText(r.approvedAmount) },
    { key: 'declinedAmount', header: 'Declined', fw: 8, align: 'right', sortable: true, cell: (r) => (r.declinedAmount ? <Money value={r.declinedAmount} /> : <NotApplicable />), text: (r) => moneyText(r.declinedAmount) },
    { key: 'declinePercent', header: 'Decline %', fw: 7, align: 'right', sortable: true, cell: (r) => `${r.declinePercent.toFixed(2)}%` },
    { key: 'totalAuth', header: 'Total Auth', fw: 9, align: 'right', sortable: true, cell: (r) => <Money value={r.totalAuth} />, text: (r) => moneyText(r.totalAuth) },
    { key: 'dav', header: 'DAV', fw: 7, align: 'right', sortable: true, cell: (r) => (r.dav ? <Money value={r.dav} /> : <Muted>—</Muted>), text: (r) => moneyText(r.dav), description: 'Daily Average Volume variance against the approved limit' },
    { key: 'dpv', header: 'DPV', fw: 7, align: 'right', sortable: true, cell: (r) => (r.dpv ? <Money value={r.dpv} /> : <Muted>—</Muted>), text: (r) => moneyText(r.dpv), description: 'Daily Processing Volume variance against the approved limit' },
...
function BatchTab() {
  ...
  const columns = [
    ...
    { key: 'txnCount', header: 'Txn Count', fw: 8, align: 'right', sortable: true },
